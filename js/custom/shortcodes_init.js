...
function initShortcodes(container) {
	...
	// Contact form
	if (container.find('.sc_contact_form:not(.sc_contact_form_contact_1) .sc_contact_form_submit:not(.inited)').length > 0) {
		container.find(".sc_contact_form:not(.sc_contact_form_contact_1) .sc_contact_form_submit:not(.inited)")
			.addClass('inited')
			.on("click", function(e){
...
function Axiom_message(opt) {
	...
	if (callback != null) {
		...
		jQuery('.Axiom_message_button').on("click", function(e) {
			"use strict";
...
function custom_menu_init(){
	...
	jQuery('.menu-panel ul.sub-menu li a').on({
		...
		mouseleave: function() {
			/*
			if (Axiom_custom_menu_holderItem) {
				Axiom_custom_menu_holderItem.html(Axiom_custom_menu_placeholder);
			}
			*/
		}
	});

	var Axiom_custom_menu_hover_timeout = 0;
